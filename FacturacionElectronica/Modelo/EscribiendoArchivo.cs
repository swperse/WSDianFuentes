...
namespace FacturacionElectronica.Controlador
{
    class EscribiendoArchivo
    {
        ...
        public String Escribir(String NroFactura, string tipoFactura, string resolucion)
        {
            ...
            alerta.Mostrar("Escribiendo " + titulo, "Generando los archivos de la " + titulo + " " + NroFactura, 1000);
            try
            {
                ...
                //  Líneas de detalle
                int numdetails = listado.Count;
                int Acum_Costos = 0;

                for (int i = 0; i < numdetails; i++)
                {
                    if (miFactura.TipoOperacion.Equals("11"))
                        listado[i].Referencia = (Convert.ToInt32(listado[i].Referencia) + Acum_Costos).ToString();

                    Cadena += listado[i].toXML(tipoFactura,
                                               miAdquiriente.Nit,
                                               miAdquiriente.DV,
                                               miFactura.TipoMoneda,
                                               Cliente.DV,
                                               Cliente.Identificacion,
                                               miFactura.TipoOperacion,
                                               miFactura.TipoFactura,
                                               listaRete) + Environment.NewLine;

                    //  En caso de ser mandato y tener costo de proveedor:
                    if (miFactura.TipoOperacion.Equals("11") && listado[i].Costo > 0)
                    {
                        listado[i].ValorProveedorPorRegistrar = true;
                ...
                //  Se ajusta el LineCountNumeric si es mandato
                {
                    
                    //var test = Cadena.Replace("<cbc:LineCountNumeric>2</cbc:LineCountNumeric>", $"<cbc:LineCountNumeric>{Length}</cbc:LineCountNumeric>");

                    int sumCostos = 0;
                    for (int i = 0; i < listado.Count; i++)
                    {
                        sumCostos += listado[i].Costo;
                    }

                    if (miFactura.TipoOperacion.Equals("11") && sumCostos > 0)
                    {
                        int countLineasDetalle = listado.Count;

                        string toreplace = "";

                        for (int i = 0; i < int.MaxValue; i++)
                        {
                            if (Cadena.Contains($"<cbc:LineCountNumeric>{i}</cbc:LineCountNumeric>"))
                            {
                                toreplace = $"<cbc:LineCountNumeric>{i}</cbc:LineCountNumeric>";
                                break;
                            }
                                
                        }

                        //  Se obtiene el número de líneas de detalle a través de la fórmula:
                        string[] t = new string[1]; t[0] = "<cac:InvoiceLine>";
                        int Length = (Cadena.Split(t, StringSplitOptions.None).Length - 1);
                        //

                        Cadena = Cadena.Replace(toreplace, $"<cbc:LineCountNumeric>{Length}</cbc:LineCountNumeric>");
                ...
                alerta = new Alerta();
                alerta.Mostrar("Escribiendo " + titulo, "La " + titulo + " " + NroFactura + " fué escrita correctamente", 1000);
                alerta.Cerrar();
                return bodyName;
            }
            catch (Exception ex)
